<?php include('partials-front/menu.php'); ?>
    ...
    <?php 
              //   $search = $_POST['search'];  // bu bizim eski kodumuz....
  ...
  <h2 class="text-center">Lebensmittel auf Ihrer Suche <a href="#" >"<?php  echo $search ; ?>"</a></h2>
...
$foods = $db->query("SELECT * FROM tbl_food WHERE (title LIKE '%$search%' OR description LIKE '%$search%') AND active='Yes' ORDER BY CAST(SUBSTRING_INDEX(food_code, ' ', 1) AS UNSIGNED), SUBSTRING(food_code, LOCATE(' ',food_code)+1);  ",PDO::FETCH_OBJ)->fetchAll();
        ...
        <div class="p-3 rounded-4 shadow-lg bg-body-tertiary">
            <div class="row">
                ...
                <?php 
                    if ($food->img_name=="") {
                        //resim yok
                        echo "<div class='error'>Resim bulunamadı</div>";
                    }
                    else
                    {
                        //resim var
                    ...
                    <div class="modal-body"> 
                        ...
                        if(count($products)>0){
                            ...
                            foreach($products as $product => $product_key){
                                ?>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="product" value="<?php echo $product_key->id; ?>" id="flexCheck<?php echo $product_key->id; ?>">
                                    <label class="form-check-label" for="flexCheck<?php echo $product_key->id; ?>">
