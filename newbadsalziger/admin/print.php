...
class PDF extends FPDF{
	...
	function firstHeader(){
		...
		$this->Image('../images/newlogo.png',40,2,80);
		$this->SetXY(1,20);
...
if ($cart[0]->order_type == "delivery") {
	$pdf->SetX(1);
	 $pdf->MultiCell(44,4,iconv('UTF-8','ISO-8859-9//TRANSLIT',"Adress : ".$customer->customer_address." - ".$customer->customer_zip." / ".$customer->customer_mahalle),0,1);
}

$pdf->Ln(4);
$pdf->SetFont('arial','B',20);
$pdf->SetX(1);
$pdf->Cell(44,5, strtoupper($cart[0]->order_type),0,1,'C');
...
foreach($cart as $item){
	...
	if($pdf->GetStringWidth($food[0]->title."(#)".$food[0]->food_code) < $cellWidth){//Burası karakter sayısının uzunluğu
		...
		$textLength = strlen($food[0]->title."(#)".$food[0]->food_code); // total text length
		...
		while($startChar < $textLength){ // loop until end of the text
			...
			while(
				...
				($startChar+$maxChar)< $textLength ){
					$maxChar++;
					$tmpString=substr($food[0]->title."(#)".$food[0]->food_code,$startChar, $maxChar); 
	...
	$xPos = $pdf->GetX();
	$yPos = $pdf->GetY();
	$pdf->MultiCell($cellWidth, $celHeight, iconv('utf-8','ISO-8859-1',$food[0]->title."(#".$food[0]->food_code.")"),0,0 );
